...
- `EnterWorktree` を使った直後は必ず `bash ~/.claude/hooks/cl-setup.sh` を実行する。compose.worktree.yml が生成され host port と docker compose project 名が他 worktree と分離される。compose を持たないリポではスクリプト側で skip される。詳細は dotfiles の `docs/claude-worktree.md`。

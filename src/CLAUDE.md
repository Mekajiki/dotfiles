...
- AI（私）が外部に投稿するコメント（PR レビュー，issue コメント等）は，AI 発信だと分かるように明示する．人手のレビュー・コメントと区別できるようにするため．書式はコメント本文の冒頭に `🤖 <` を置き，ロボットが喋っている体にする（例: `🤖 < ここの null チェックは握りつぶしてる`）．`(AIによるレビューです)` のような冗長な注釈は付けない——絵文字マーカーだけで十分伝わる．（これは AI 発信を示す意図的なマーカーであり，上の「装飾記号を使わない」＝列挙・参照番号の装飾禁止 とは別物）

...
- linter向けの除外の追加は基本的に不可．長さ系だけは無意味な細切れにするよりは許容．それ以外で必要と判断した場合は勝手に足さず確認を取る．
- 丸数字（①②③…）など非ANSI（非ASCII）の装飾記号を使わない．列挙や参照の番号・記号は ANSI文字で書けるものなら何でもよい（`1.` `(1)` `1)` `1点目` 等）．コミット・PR・レビュー・チャット応答すべてに適用する．
- AI（私）が外部に投稿する文章（issue の本文，issue コメント，PR レビュー等）は，AI 発信だと分かるように明示する．人手のものと区別できるようにするため．コミットと PR 本文は定型署名（`Co-Authored-By` / `Generated with Claude Code`）で判別できるので対象外．書式は本文の冒頭に `🤖 <` を置き，ロボットが喋っている体にする（例: `🤖 < ここの null チェックは握りつぶしてる`）．`(AIによるレビューです)` のような冗長な注釈は付けない——絵文字マーカーだけで十分伝わる．（これは AI 発信を示す意図的なマーカーであり，上の「装飾記号を使わない」＝列挙・参照番号の装飾禁止 とは別物）

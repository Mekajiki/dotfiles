...
hooks の設定は `claude-settings.local.json` に書いてある。Claude 本体は `settings.local.json` には書き込まないので symlink で安全。`settings.json` の hooks 配列とマージされて両方走る。

新規/既存ユーザー区別なく、`./link_files.sh` を再実行するだけで hook の追加・更新が反映される。手動マージ作業は不要。
